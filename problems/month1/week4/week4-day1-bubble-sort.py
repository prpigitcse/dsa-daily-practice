...
Approach:
1. Outer loop: run from i = 0 to n-1 (n passes).
...
5. Recursive version: replace the outer loop with recursion — each recursive call represents one pass, with i incrementing.
...
Common Mistakes:
- Forgetting to reduce the inner loop range with each pass (the last i elements are already sorted)
- Not using a "swapped" flag for early termination — if no swaps occur in a pass, the array is already sorted
- Confusing the indices in the recursive version, leading to incorrect or infinite recursion
- Using Bubble Sort for large arrays in production — it is O(N^2) and impractical beyond ~10,000 elements
...
# Given an unsorted array of integers, sort it in ascending order using the Bubble Sort algorithm. Modify the array in-place (O(1) space).
